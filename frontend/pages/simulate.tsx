...
import { AGENT_TYPES, ACTION_LABELS } from '@/lib/constants'
import type { AgentStats, HandSummary, SimulationResult } from '@/types'
...
export default function SimulatePage() {
  ...
  const runSimulation = async () => {
    ...
    try {
      const res = await simulate({ agent_1: agent1, agent_2: agent2, num_hands: numHands })
      setResult(res)
  ...
  return (
    <div className="max-w-6xl">
      <div className="mb-6">
        ...
        <p className="text-gray-400 text-sm mt-1">Pit agents against each other over thousands of hands</p>
      </div>
      ...
      <div className="bg-gray-900 rounded-xl border border-gray-800 p-4 mb-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-x-6 gap-y-2 text-sm">
          <div><span className="text-amber-400 font-medium">TAG</span> <span className="text-gray-500">— Tight-Aggressive. Few hands, bets hard.</span></div>
          <div><span className="text-amber-400 font-medium">LAG</span> <span className="text-gray-500">— Loose-Aggressive. Many hands, frequent bluffs.</span></div>
          <div><span className="text-amber-400 font-medium">CallingStation</span> <span className="text-gray-500">— Calls everything, rarely raises.</span></div>
          <div><span className="text-amber-400 font-medium">Random</span> <span className="text-gray-500">— Random legal action. Noise baseline.</span></div>
          <div><span className="text-amber-400 font-medium">MLAgent</span> <span className="text-gray-500">— Uses RF model to pick highest-EV action.</span></div>
          <div><span className="text-amber-400 font-medium">Custom</span> <span className="text-gray-500">— Strategy config from Strategy Lab.</span></div>
        </div>
      </div>
      ...
      <div className="bg-gray-900 rounded-xl border border-gray-800 p-6 mb-6">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 items-end">
          ...
          <div>
            <label className="text-xs text-gray-500 block mb-1">Hands: {numHands.toLocaleString()}</label>
            <input type="range" min="100" max="10000" step="100" value={numHands}
      ...
      {result && (
        <div className="space-y-6">
            ...
            <ResponsiveContainer width="100%" height={350}>
              <LineChart data={chartData} onClick={handleChartClick} style={{ cursor: 'crosshair' }}>
                ...
                <YAxis
                  ...
                  label={{ value: 'Profit (BB)', angle: -90, position: 'insideLeft', fill: '#6b7280', fontSize: 11 }}
                />
                <Tooltip
                  contentStyle={{ backgroundColor: '#111827', border: '1px solid #374151', borderRadius: '8px', fontSize: 13 }}
                  labelFormatter={(v) => `Hand #${v}`}
                  formatter={(value, name) => [`${Number(value) >= 0 ? '+' : ''}${Number(value).toFixed(1)} BB`, name]}
                />
                <Legend wrapperStyle={{ fontSize: 12 }} />
                <ReferenceLine y={0} stroke="#374151" />
                <Line type="monotone" dataKey="agent_1_bankroll" stroke="#10b981" name={agent1} dot={false} strokeWidth={2} />
                <Line type="monotone" dataKey="agent_2_bankroll" stroke="#f59e0b" name={agent2} dot={false} strokeWidth={2} />
              </LineChart>
            </ResponsiveContainer>
          </div>
                  ...
                  <div>
                    <p className="text-xs text-gray-500 mb-1">
                      ...
                      <span className={clsx('ml-2 font-mono text-sm',
                        ...
                        {selectedHand.agent_1_profit_bb > 0 ? '+' : ''}{selectedHand.agent_1_profit_bb} BB
                      </span>
                    </p>
                  ...
                  <div>
                    <p className="text-xs text-gray-500 mb-1">
                      ...
                      <span className={clsx('ml-2 font-mono text-sm',
                        ...
                        {selectedHand.agent_2_profit_bb > 0 ? '+' : ''}{selectedHand.agent_2_profit_bb} BB
                      </span>
                    </p>
                      ...
                      return (
                        <div key={i}>
                          ...
                          <div className="flex items-center gap-2 text-sm py-0.5">
                            ...
                            {a.amount_bb > 0 && (
                              <span className="text-gray-500 font-mono text-xs">{a.amount_bb} BB</span>
                            )}
                          </div>
                        </div>
                      )
          ...
          {/* Stats Tables */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <StatsTable title={agent1} stats={result.agent_1_stats} color="text-emerald-400" />
            <StatsTable title={agent2} stats={result.agent_2_stats} color="text-amber-400" />
          </div>
        </div>
      )}
    </div>
...
function StatsTable({ title, stats, color }: { title: string; stats: AgentStats; color: string }) {
  const rows: [string, string][] = [
    ['BB/100', `${stats.bb_per_100?.toFixed(1)} BB`],
    ...
    ['Total Profit', `${stats.total_profit_bb?.toFixed(1)} BB`],
  ]
